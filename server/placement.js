...
export function placeResourceInEnvironment({ user, type, resourceUuid, environmentId, kind }) {
  ...
  // Kind to store: a caller-derived `kind` (from the live resource) wins. Otherwise a
  // database must still carry a DB kind (never web_service), so default it to postgres;
  // apps are left untouched on UPDATE so a previously-derived static_site/worker survives.
  ...
  const forceKind = kind || (type === "database" ? "postgres" : null);
  const res = forceKind
    ? db.prepare("UPDATE resource_ownership SET environment_id = ?, org_id = ?, kind = ? WHERE type = ? AND coolify_uuid = ?").run(env.id, env.org_id, forceKind, type, resourceUuid)
  ...
  if (!res.changes) {
    ...
    db.prepare(`INSERT INTO resource_ownership (coolify_uuid, type, user_id, org_id, environment_id, kind, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?)`)
      .run(resourceUuid, type, user.id, env.org_id, env.id, forceKind || "web_service", new Date().toISOString());
  }
  return { ok: true };
}
