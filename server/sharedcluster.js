...
// pinned pg client container (PG_IMAGE) on the Coolify network (needs the Docker
// socket, the same prereq as the pg_dump data copy).
import { randomBytes } from "node:crypto";
import * as coolify from "./coolify.js";
import { getSetting, setSetting } from "./db.js";
import { encryptSecret, decryptSecret } from "./secretbox.js";
import { dockerPg, runOnHost, PG_IMAGE } from "./hostexec.js";
...
async function waitForPg(url) {
  ...
  for (let i = 0; i < 24; i++) {
    try {
      await runOnHost(`docker run --rm --network ${net} ${PG_IMAGE} pg_isready -h ${host} -p 5432 -t 5`);
      return;
...
// Run SQL against the cluster in a pinned PG_IMAGE container on the host. URL +
